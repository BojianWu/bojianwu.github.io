/** @file proc-db/brep_surf.cpp
 *
 * Creating a single NURBS surface w/o thickness value
 */
...
Cobb_InnerTrimmingLoop(ON_Brep& brep, ON_BrepFace& face,
    int UNUSED(vSWi), int UNUSED(vSEi), int UNUSED(vNEi), int UNUSED(vNWi),
    int eSi, int eS_dir,
    int eEi, int eE_dir,
    int eNi, int eN_dir,
    int eWi, int eW_dir)
...
main(int argc, char** argv)
{
    ...
    const char* id_name = "B-Rep Surface";
    const char* geom_name = "surf.s";
    ...
    outfp = wdb_fopen("brep_surf_trimmed.g");
    mk_id(outfp, id_name);
    ...
    mk_region1(outfp, "surf.r", geom_name, "plastic", "", rgb);
