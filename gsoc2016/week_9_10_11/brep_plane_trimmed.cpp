 ...
 * 
 * Generating a single B-rep surface with trimmed edges.
 */
...
int main(int argc, char *argv[])
{
    ...
    const char* geom_name = "plane.s";

    ON::Begin();

    /* export brep to file */
    bu_log("Writing a b-rep surface...\n");
    outfp = wdb_fopen("brep_plane_trimmed.g");
    mk_id(outfp, id_name);

    brep = MakeBrepFace(error_log);
    mk_brep(outfp, geom_name, brep);

    unsigned char rgb[] = {50, 255, 50};
    mk_region1(outfp, "plane.r", geom_name, "plastic", "", rgb);
